...
if(strlen($_SESSION['alogin']) == 0) {	
    ...
    if(isset($_POST['update'])) {
        $id = intval($_GET['imgid']); // pastikan ID adalah angka

        // Sanitasi nama file
        $vimage = basename($_FILES["img2"]["name"]);
        $vimage = htmlspecialchars($vimage, ENT_QUOTES, 'UTF-8');

        // Upload file ke folder yang ditentukan
        $uploadPath = "img/vehicleimages/" . $vimage;
        if(move_uploaded_file($_FILES["img2"]["tmp_name"], $uploadPath)) {
            // Query update menggunakan bindParam
									...
									<div class="panel-body">
										<form method="post" class="form-horizontal" enctype="multipart/form-data">
											...
											<div class="form-group">
												...
												<?php 
$id = isset($_GET['imgid']) ? intval($_GET['imgid']) : 0; // validasi angka
...
if ($query->rowCount() > 0) {
    foreach ($results as $result) { ?>
        <div class="col-sm-8">
            <img src="img/vehicleimages/<?php echo htmlspecialchars($result->Vimage2); ?>" width="300" height="200" style="border:solid 1px #000">
        </div>
<?php }
} ?>
											</div>
