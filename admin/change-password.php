...
if(strlen($_SESSION['alogin']) == 0) {	
    ...
    if(isset($_POST['submit'])) {
        $password = $_POST['password'];           // Password lama (plain text input)
        $newpassword = $_POST['newpassword'];     // Password baru (plain text input)
        ...
        if ($result) {
            ...
            // Cek apakah password tersimpan masih plaintext
            if ($password === $stored_password || password_verify($password, $stored_password)) {
                // Simpan password baru dalam bentuk bcrypt
                ...
                $query->bindParam(':newpassword', $newpassword_hashed, PDO::PARAM_STR);
                $query->bindParam(':username', $username, PDO::PARAM_STR);
                $query->execute();

                $msg = "Password berhasil diubah dan diamankan dengan bcrypt.";
            } else {
